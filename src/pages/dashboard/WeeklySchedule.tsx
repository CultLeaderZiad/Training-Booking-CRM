...
export default function WeeklySchedule() {
  ...
  const filteredSessionTypes = sessionTypes.filter(st => {
    ...
    const matchesCategory = activeCategory === 'ALL' || st.session_categories?.name?.toUpperCase() === activeCategory;
    return matchesSearch && matchesCategory;
  });

  const categories = ['ALL', ...new Set(sessionTypes.map(st => st.session_categories?.name?.toUpperCase()).filter(Boolean))];
           ...
           <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredSessionTypes.map(st => (
                <div key={st.id} className="group bg-[#1A1D24] border border-border/50 rounded-[32px] p-8 flex flex-col hover:border-[#f97316]/30 transition-all duration-300 relative overflow-hidden">
                   ...
                   <div className="mb-8">
                      <Badge className="bg-[#f97316]/10 text-[#f97316] border-none text-[8px] font-black uppercase px-2 py-0.5 mb-3">{st.session_categories?.name || 'General'}</Badge>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="hidden lg:block w-80 shrink-0">
              <div className="sticky top-24 space-y-6">
                <div className="bg-[#1A1D24] border border-border/50 rounded-[32px] p-6">
                ...
                <div className="bg-gradient-to-br from-[#f97316]/20 to-transparent border border-[#f97316]/10 rounded-[32px] p-6">
                  <h4 className="text-xs font-black text-[#f97316] uppercase tracking-[0.2em] mb-2">Need Help?</h4>
                  <p className="text-[10px] text-gray-500 font-medium leading-relaxed">
                    Can't find a session that fits your schedule? Contact our team for personalized training options.
                  </p>
                  <Button variant="link" className="text-[#f97316] text-[10px] font-black uppercase p-0 mt-2 h-auto">
                    Contact Coach →
                  </Button>
                </div>
              </div>
